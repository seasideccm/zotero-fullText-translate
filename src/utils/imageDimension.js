function resolution (data){
    if(isBase64Img(data)){
        ...
        const extArr =['png', 'jpg', 'gif', 'bmp']
        const ext=extArr.find(e=>baseSplit[0].includes(e))
        return ReadBase64Dimension(data,ext)
    } else if (isBytes(data)){
        return ReadBytesDimension(data)
    }
...
function isBase64Img(str){
    ...
    const  baseSplit = str.split(',');
    const strBase64 = baseSplit[1]
    return isBase64(strBase64)
...
//https://github.com/sindresorhus/image-dimensions/tree/main
var IMAGE_HEAD_SIGS = {
...
function ReadBytesDimension(bytes) {
    let resolution
    resolution =ReadPNG(bytes)
    if(resolution) return resolution
    resolution =ReadJPG(bytes)
    if(resolution) return resolution
    resolution =ReadGIF(bytes)
    if(resolution) return resolution
    resolution =ReadBMP(bytes)
    if(resolution) return resolution    
}
...
/* export {
    ...
    ReadBase64Dimension,ReadBytesDimension,resolution
} */
